...
int main(int argc, char** argv) {
 ...
 const char *network_hdf5 = argv[3];

//  hid_t file_id = H5Fcreate(network_hdf5, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);

 shared_ptr<Net> caffe_net;
 caffe_net.reset(new Net(network_params, caffe::TEST));
...
//   }
 ...
 caffe_net->ToProto(&net_param, true);
 caffe_net->ToHDF5(network_hdf5, false);
// WriteProtoToBinaryFile(net_param, network_snapshot);
 //  H5Fclose(file_id);
  return 0;
 }

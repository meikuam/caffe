...
BoxDataLayer<Ftype, Btype>::BoxDataLayer(const LayerParameter& param, size_t solver_rank)
  : BasePrefetchingDataLayer<Ftype, Btype>(param, solver_rank),
    reader_(param) {
}

template <typename Ftype, typename Btype>
BoxDataLayer<Ftype, Btype>::~BoxDataLayer() {
  this->StopInternalThread();
}

template <typename Ftype, typename Btype>
void BoxDataLayer<Ftype, Btype>::DataLayerSetUp(const vector<Blob*>& bottom,
  ...
  Datum& datum = *(reader_.full_peek());

  // Use data_transformer to infer the expected blob shape from datum.
  vector<int> top_shape = this->fwd_data_transformers_[0]->InferBlobShape(datum);
  ...
  if (this->output_labels_) {
    ...
    for (int i = 0; i < sides_.size(); ++i) {
      ...
      for (int j = 0; j < top.size() - 1; ++j) {
        shared_ptr<TBlob<Ftype> > tmp_blob;
        tmp_blob.reset(new TBlob<Ftype>(label_shape));
...
template <typename Ftype, typename Btype>
void BoxDataLayer<Ftype, Btype>::load_batch(Batch *batch) {
...
template <typename Ftype, typename Btype>
void BoxDataLayer<Ftype, Btype>::transform_label(int count, Ftype *top_label,
...
INSTANTIATE_CLASS_FB(BoxDataLayer);

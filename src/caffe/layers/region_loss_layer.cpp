...
void RegionLossLayer<Ftype, Btype>::LayerSetUp(
...
template <typename Ftype, typename Btype>
void RegionLossLayer<Ftype, Btype>::Reshape(const vector<Blob*> &bottom, const vector<Blob*>& top) {
...
template <typename Ftype, typename Btype>
void RegionLossLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*> &bottom, const vector<Blob*> &top) {
    //const Dtype* input_data = bottom[0]->cpu_data();
    //std::cout<<"1"<<std::endl;
    const Ftype* label_data = bottom[1]->cpu_data<Ftype>(); //[label,x,y,w,h]
    //std::cout<<"2"<<std::endl;
    Ftype* diff = diff_.mutable_cpu_data();
    caffe_set(diff_.count(), Ftype(0.0), diff);
    //std::cout<<"3"<<std::endl;
    Ftype avg_anyobj(0.0), avg_obj(0.0), avg_iou(0.0), avg_cat(0.0), recall(0.0), loss(0.0);
    int count = 0;
    int class_count = 0;
    //*********************************************************Reshape********************************************************//
    TBlob<Ftype> swap;
    ...
    Ftype* swap_data = swap.mutable_cpu_data();
    int index = 0;
   ...
   if (softmax_tree_ != ""){
    ...
    for (int b = 0; b < swap.num(); ++b){
      if (softmax_tree_ != ""){
        ...
        for (int t = 0; t < 30; ++t){
          vector<Ftype> truth;
          Ftype x = label_data[b * 30 * 5 + t * 5 + 1];
          Ftype y = label_data[b * 30 * 5 + t * 5 + 2];
          if (!x) break;
	...
	//LOG(INFO)<< "tree ok";
      ...
      for (int j = 0; j < side_; ++j)
        ...
        vector<Ftype> pred = get_region_box(swap_data, biases_, n, index, i, j, side_, side_);
            float best_iou = 0;
            for (int t = 0; t < 30; ++t){
          vector<Ftype> truth;
              Ftype x = label_data[b * 30 * 5 + t * 5 + 1];
          Ftype y = label_data[b * 30 * 5 + t * 5 + 2];
          Ftype w = label_data[b * 30 * 5 + t * 5 + 3];
          Ftype h = label_data[b * 30 * 5 + t * 5 + 4];
    ...
    Ftype* real_diff = real_diff_.mutable_cpu_data();
    //std::cout<<"9"<<std::endl;
    int sindex = 0;
...
template <typename Ftype, typename Btype>
void RegionLossLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*> &top,
  ...
  if (propagate_down[0]) {
    const Ftype sign(1.);
    const Ftype alpha = sign * top[0]->cpu_diff<Btype>()[0] / bottom[0]->num();
...
INSTANTIATE_CLASS_FB(RegionLossLayer);
